...
class V2TransfuserModel(nn.Module):
    ...
    def forward(self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor]=None, eta=0.0, metric_cache=None, cal_pdm=True,token=None) -> Dict[str, torch.Tensor]:
        ...
        # print(f"🛑🛑🛑metric_cache_path in diffusiondrivev2_model_sel--V2Transformer_forward  is{metric_cache}!")
        ...
        # print(f"🛑🐷 metric_cache_path in diffusiondrivev2_model_sel--【forward 】 before trajectory head is{metric_cache}!")
...
class TrajectoryHead(nn.Module):
    ...
    def forward(self, ego_query, agents_query, bev_feature,bev_spatial_shape,status_encoding,status_feature,camera_feature, targets=None,global_img=None,eta=0.0,metric_cache=None, cal_pdm=True, old_model=False,token=None) -> Dict[str, torch.Tensor]:
        """Torch module forward pass."""
        # print(f"🛑🐷 metric_cache_path in diffusiondrivev2_model_sel--【forward 】 is{metric_cache}!")
    ...
    def get_pdm_score_para(self, trajectory, metric_cache_path):
        ...
        #DEBUG
        # support None/str/Path inputs by broadcasting to batch
        if metric_cache_path is None:
            raise RuntimeError("🐷metric_cache_path is None. Provide per-batch cache paths (list/tuple) or a single path.")
        # if a single path is provided, broadcast to batch size
        if isinstance(metric_cache_path, (str, os.PathLike)):
            metric_cache_path = [metric_cache_path] * B
        # basic validation
        if not isinstance(metric_cache_path, (list, tuple)) or len(metric_cache_path) < B:
            raise RuntimeError(f"🐷metric_cache_path must be a list/tuple with at least {B} entries.")
        #DEBUG
    ...
    def forward_test_rl(self, ego_query,agents_query,bev_feature,bev_spatial_shape,status_encoding,status_feature,camera_feature, targets,global_img,metric_cache,eta=1.0,token=None) -> Dict[str, torch.Tensor]:
        ...
        # print(f"🛑🐷 metric_cache_path in diffusiondrivev2_model_sel--forward_test_rl  is{metric_cache}!")
