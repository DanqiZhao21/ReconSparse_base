...
from framework.algorithms.nuscenes_scorer_utils import (
...
class NuScenesPDMScorer:
    ...
    This stage wraps the shared NuScenes utility layer with a reusable sample-level
    context object that mirrors the structure needed for batch occupancy and
    polygon-query based metrics.
    """
    ...
    def _build_object_geometry_arrays(
        ...
        for idx, obj in enumerate(scene_objects):
            ...
            if corners is None:
                ...
                corners_xy = NuScenesScorerUtils._box_corners_xy(
    ...
    def _batch_centerline_stats(
        ...
        *,
        ...
        if direction_xy is not None:
            ...
            if direction.shape != pts.shape:
                raise RuntimeError(f"Expected direction_xy shape {tuple(pts.shape)}, got {tuple(direction.shape)}")
        ...
        if direction is not None:
            direction_norm = np.linalg.norm(direction, axis=-1).astype(np.float32, copy=False)
            unit_direction = np.where(
                direction_norm[..., None] > 1.0e-6,
                direction / np.maximum(direction_norm[..., None], 1.0e-6),
                0.0,
            ).astype(np.float32, copy=False)
            tangent_dot = np.sum(unit_direction[:, :, None, :] * tangents_xy[None, None, :, :], axis=-1)
            same_dir_mask = (
                (direction_norm[:, :, None] > 1.0e-6)
                & valid_seg[None, None, :]
                & (tangent_dot >= float(same_dir_dot_threshold))
            )
            same_dir_dist = np.where(same_dir_mask, dist, np.inf).astype(np.float32, copy=False)
            same_dir_idx = np.argmin(same_dir_dist, axis=-1)
            same_dir_best_dist = np.take_along_axis(same_dir_dist, same_dir_idx[..., None], axis=-1).squeeze(-1)
            use_same_dir = np.isfinite(same_dir_best_dist) & (
                same_dir_best_dist <= best_dist + max(0.0, float(same_dir_distance_margin_m))
            )
            selected_idx = np.where(use_same_dir, same_dir_idx, best_idx)
            selected_dist = np.where(use_same_dir, same_dir_best_dist, best_dist).astype(np.float32, copy=False)

        best_tangent = tangents_xy[np.clip(selected_idx, 0, max(0, tangents_xy.shape[0] - 1))].astype(np.float32, copy=False)
        finite_mask = np.isfinite(selected_dist)
        best_dist = np.where(finite_mask, selected_dist, 0.0).astype(np.float32, copy=False)
    ...
    def _batch_collision_ttc_metrics(
        ...
        *,
        ...
        if horizon <= 0 or len(sample_context.occupancy_map) <= 0:
            if len(sample_context.ttc_agent_states) <= 0:
                return {
                    ...
                    "ttc": ttc,
                    "collision_matrix": np.zeros((num_candidates, horizon), dtype=bool),
                }
        ...
        return {
            "no_collision": no_collision,
            "ttc": ttc,
            "collision_matrix": collision_hits.astype(bool, copy=False),
        }
