...
def test_scene_index_reports_remaining_future_sample_count(tmp_path: Path):
    nusc = tmp_path / "nuscenes" / "v1.0-trainval"
    recon_info = tmp_path / "assets" / "nus" / "information" / "frame2token"
    ...
    assert index.remaining_future_sample_count("scene-0013", 0) == 2
    assert index.remaining_future_sample_count("scene-0013", 1) == 1
    assert index.remaining_future_sample_count("scene-0013", 2) == 0
