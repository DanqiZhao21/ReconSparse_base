        ...
        *,
        ...
        return {
            ...
            "counterfactual": {
                "traj_xyyaw": torch.zeros((2, 3, 4, 3), dtype=torch.float32),
                "log_probs": torch.full((2, 3), -0.5, dtype=torch.float32),
                "old_log_probs": torch.full((2, 3), -0.6, dtype=torch.float32),
            },
        }
...
def test_grpo_only_uses_actor_stored_old_candidate_log_probs(monkeypatch) -> None:
    ...
    learner_config = ActorLearnerLightningConfig(
        ...
        grpo_coef=1.0,
        grpo_num_candidates=3,
        grpo_candidate_select="sample_old_policy",
        grpo_norm_eps=1.0e-6,
        grpo_use_rank_adv=False,
        grpo_score_clip=None,
        grpo_objective="grpo",
    ...
    def _fake_grpo_objective(**kwargs):
        seen.update(kwargs)
        return TrajectoryGRPOObjective(
            ...
            approx_kl=torch.tensor(0.1, dtype=torch.float32),
            clip_frac=torch.tensor(0.25, dtype=torch.float32),
            ratio_mean=torch.tensor(1.05, dtype=torch.float32),
        )

    monkeypatch.setattr("framework.lightning.trajectory_module.compute_grpo_objective", _fake_grpo_objective)
    ...
    assert module.latest_metrics["grpo_clip_frac"] == torch.tensor(0.25, dtype=torch.float32).item()
    assert module.latest_metrics["grpo_ratio_mean"] == torch.tensor(1.05, dtype=torch.float32).item()
