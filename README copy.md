...
- **权重版本（`outputs/actor_learner/weights/version.txt`）**: Learner 每次更新后由 rank0 保存 `latest.ckpt` 并将 `version.txt` 加 1。Actor 在写入 shard 前会尝试加载最新版本；在 `mode: sync` 下，Actor 写 shard 后会等待版本更新再继续，避免不同步导致的超时。
- **Shard 消费机制**: Learner 选定本次更新使用的 shard 文件名，通过 DDP 广播；更新完成后由 rank0 将这些文件移动到 `consumed/`，防止重复训练。若 `shards_per_update` 过大或 `mode: async` 下 Learner 长时间未选 shard，可能造成堆积。
- **DDP 超时（NCCL/Gloo）**: 典型原因是 rank 间在 `broadcast/barrier` 前后未达到一致状态（例如未及时写入/选择 shard）。建议：
	- 将 `actor_learner.mode` 设为 `sync` 并降低 `shards_per_update`；
	- 适当调小 `num_envs_per_actor` 以提高 Learner 更新频率；
	- 配置 `train.ddp.timeout_s`（如 900s）避免超时退出；
	- 日志中若出现长时间等待，优先检查 `outputs/actor_learner/shards/` 文件数量与 `version.txt` 是否递增。

## 统一框架（framework/*）

- **Agent 接口**: `framework/agent/base.py`，DDV2 适配器见 `framework/agent/diffusiondrive_v2.py`。
- **算法接口与实现**:
	- PPO 包装器：`framework/algorithms/ppo.py`（底层核心在 `framework/algorithms/ppo_ddv2_core.py`）
	- Reinforce++ 包装器：`framework/algorithms/reinforcepp.py`（底层核心在 `framework/algorithms/reinforcepp_core.py`）
- **缓冲区 IO**: `framework/io/buffer.py`（统一引入 `BufferPaths` 与 shard/版本管理）。
- **训练入口**: 使用新版脚本 `script/train_actor_learner_v2.py`。

### 运行示例

```bash
# Reinforce++ 默认（或显式配置）
ALGO=reinforcepp LOG_DIR=./logs bash tools/train_actor_learner.sh
# 或
CONFIG=script/configs/ppo_closed_loop.yaml LOG_DIR=./logs bash tools/train_actor_learner.sh
```

> 提示：逐步迁移中，`framework/*` 目前以门面形式重导出 `rl/*` 关键实现，后续将继续内聚到 `framework` 下，保持对外 API 不变。
