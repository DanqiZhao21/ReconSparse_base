...
def default_run_specs() -> list[RunSpec]:
    ...
    return [
        RunSpec(
            run_key="reinforcepp_baseline_reward_dac_weight_grpo_coef003",
            coef=0.03,
            config_path=cfg_root
            / "reinforcepp_closed_loop_sparsedrive_v2_baseline_reward_dac_weight_grpo_coef003.yaml",
        ),
        RunSpec(
            run_key="reinforcepp_baseline_reward_dac_weight_grpo_coef01",
            coef=0.1,
            config_path=cfg_root
...
def promote_checkpoint(*, src_ckpt: Path, spec: RunSpec, date_tag: str) -> tuple[Path, int]:
    ...
    version = detect_next_version(
        ...
        run_key_prefix="reinforcepp_baseline_reward_dac_weight_grpo",
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run baseline-reward Reinforce++ plus DAC-weight/TTC GRPO coef=0.03/0.1 and HUGSIM-ORI eval."
    )
    parser.add_argument(
        "--run-root",
        type=Path,
        default=REPO_ROOT / "outputs" / "reinforcepp_baseline_reward_dac_weight_grpo_coef_sweep",
    )
...
def main() -> int:
    args = parse_args()
    run_id = args.run_id or f"{time.strftime('%Y%m%d_%H%M%S', time.gmtime())}_baseline_reward_dac_weight_grpo"
