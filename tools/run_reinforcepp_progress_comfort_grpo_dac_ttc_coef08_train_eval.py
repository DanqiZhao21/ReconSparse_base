...
def default_run_spec() -> RunSpec:
    return RunSpec(
        run_key="reinforcepp_progress_comfort_grpo_dac_ttc_coef08",
        coef=0.8,
        eval_repeats=2,
        ...
        / "configs"
        / "sparsedrive_v2"
        / "reinforcepp_closed_loop_sparsedrive_v2_progress_comfort_grpo_dac_ttc_coef08.yaml",
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Reinforce++ progress/comfort reward plus DAC/TTC GRPO coef=0.8 and two HUGSIM eval repeats."
    )
    parser.add_argument(
        "--run-root",
        type=Path,
        default=REPO_ROOT / "outputs" / "reinforcepp_progress_comfort_grpo_dac_ttc_coef08_train_eval",
    )
