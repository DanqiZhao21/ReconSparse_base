...
def default_run_specs() -> list[RunSpec]:
    ...
    return [
        RunSpec(
            run_key="reinforcepp_progress_comfort_dac_weight_grpo_coef01",
            coef=0.1,
            config_path=cfg_root
            / "reinforcepp_closed_loop_sparsedrive_v2_progress_comfort_dac_weight_grpo_coef01.yaml",
        ),
        RunSpec(
            run_key="reinforcepp_progress_comfort_dac_weight_grpo_coef08",
            coef=0.8,
            config_path=cfg_root
...
def promote_checkpoint(*, src_ckpt: Path, spec: RunSpec, date_tag: str) -> tuple[Path, int]:
    ...
    version = detect_next_version(ckpt_dir=SPARSEDRIVE_CKPT_DIR, run_key_prefix="reinforcepp_progress_comfort_dac_weight_grpo")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Reinforce++ progress/comfort reward plus DAC-weight/TTC GRPO coef=0.1/0.8 and HUGSIM-ORI eval."
    )
    parser.add_argument(
        "--run-root",
        type=Path,
        default=REPO_ROOT / "outputs" / "reinforcepp_progress_comfort_dac_weight_grpo_coef_sweep",
    )
...
def main() -> int:
    args = parse_args()
    run_id = args.run_id or f"{time.strftime('%Y%m%d_%H%M%S', time.gmtime())}_reinforcepp_progress_comfort_dac_weight_grpo_coef_sweep"
        ...
        if args.dry_run:
            promoted_ckpt = SPARSEDRIVE_CKPT_DIR / build_promoted_ckpt_name(
                ...
                version=detect_next_version(
                    ...
                    run_key_prefix="reinforcepp_progress_comfort_dac_weight_grpo",
                ),
                run_key=spec.run_key,
            )
            version = 0
