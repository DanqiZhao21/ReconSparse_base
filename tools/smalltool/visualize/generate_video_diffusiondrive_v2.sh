...
# Repo-root relative runner for generate_video_diffusiondrive_v2.py
REPO_ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)
DDV2_ROOT="$REPO_ROOT/egoADs/DiffusionDriveV2"
if [[ ! -d "$DDV2_ROOT" ]]; then
...
DEFAULT_EXECUTE_MODE=${DEFAULT_EXECUTE_MODE:-nearest_anchor}
DEFAULT_MODE_SELECT=${DEFAULT_MODE_SELECT:-greedy}
DEFAULT_ETA=${DEFAULT_ETA:-1.0}

DEFAULT_CKPT=${DEFAULT_CKPT:-"$DDV2_ROOT/ckpt/diffusiondrivev2_rl.ckpt"}
...
# Ensure repo and DiffusionDriveV2 are importable
export PYTHONPATH="$REPO_ROOT:$DDV2_ROOT:$DDV2_ROOT/navsim:${PYTHONPATH:-}"
...
AUTO_OUT_DIR="$REPO_ROOT/outputs/visualize/diffusiondriveV2"

AUTO_OUT="$AUTO_OUT_DIR/scene${EFFECTIVE_SCENE_PADDED}_${MODEL_NAME}_${TS}_diffusiondrivev2_rollout.mp4"
AUTO_TRAJ_CSV="$AUTO_OUT_DIR/scene${EFFECTIVE_SCENE_PADDED}_${MODEL_NAME}_${TS}_diffusiondrivev2_plan_frontframe.csv"
AUTO_TRAJ_PLOT="$AUTO_OUT_DIR/scene${EFFECTIVE_SCENE_PADDED}_${MODEL_NAME}_${TS}_diffusiondrivev2_expert_vs_ego_traj.svg"
...
CMD=(
  python "$REPO_ROOT/tools/smalltool/visualize/generate_video_diffusiondrive_v2.py"
  ...
  --cuda "$DEFAULT_CUDA"
  --execute-mode "$DEFAULT_EXECUTE_MODE"
...
echo "[generate_video_diffusiondrive_v2.sh] REPO_ROOT=$REPO_ROOT"
echo "[generate_video_diffusiondrive_v2.sh] DDV2_ROOT=$DDV2_ROOT"
echo "[generate_video_diffusiondrive_v2.sh] scene=$EFFECTIVE_SCENE_ID"
echo "[generate_video_diffusiondrive_v2.sh] ckpt=$EFFECTIVE_CKPT"
echo "[generate_video_diffusiondrive_v2.sh] out=$FINAL_OUT"
echo "[generate_video_diffusiondrive_v2.sh] traj_csv=$FINAL_TRAJ_CSV"
echo "[generate_video_diffusiondrive_v2.sh] traj_plot=$FINAL_TRAJ_PLOT"
...
cd "$REPO_ROOT"
echo "[generate_video_diffusiondrive_v2.sh] cwd=$(pwd)"

exec "${CMD[@]}"

# Usage examples:
# bash tools/smalltool/visualize/generate_video_diffusiondrive_v2.sh --scene=99
# bash tools/smalltool/visualize/generate_video_diffusiondrive_v2.sh --scene 99 --cuda 0
# bash tools/smalltool/visualize/generate_video_diffusiondrive_v2.sh --scene 99 --duration-s 20
# bash tools/smalltool/visualize/generate_video_diffusiondrive_v2.sh --scene 99 --execute-mode first_step