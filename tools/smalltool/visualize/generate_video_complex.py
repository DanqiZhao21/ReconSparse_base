...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "Video playback FPS. NOTE: this controls *playback speed*, not simulated time. "
            "If unset, defaults to 10. Use --realtime to match simulated seconds." 
        ),
    )
    ap.add_argument(
        "--realtime",
        action="store_true",
        help=(
            "Set video FPS automatically to match simulated time (no speed-up). "
            "If interp-method=none, fps=1/step_dt (e.g., 2 when step_dt=0.5s). "
            "If interp is enabled, fps=interp_frames_per_step/step_dt (e.g., 10 when 5 frames/step and step_dt=0.5s)."
        ),
    )
    ...
    fps_print = "auto" if args.fps is None else str(float(args.fps))
    print(f"max_steps={int(args.max_steps)} fps={fps_print} realtime={bool(args.realtime)} step_frames={step_frames}")
    ...
    for idx, sid in enumerate(scenes):
        ...
        if bool(args.realtime):
            if args.fps is not None:
                print("   warning: --realtime overrides explicit --fps", file=sys.stderr)
            frames_per_step = 1 if interp_method == "none" else int(interp_n)
            fps_used = float(frames_per_step) / float(step_dt_s)
        else:
            fps_used = 10.0 if args.fps is None else float(args.fps)
        if not (fps_used > 0.0):
            raise RuntimeError(f"Invalid fps computed: {fps_used}")
        ...
        writer = imageio.get_writer(out_path, mode="I", fps=float(fps_used), macro_block_size=1)
        frames_written = 0
        ...
        try:
            ...
            pass
        _append(_grid_frame(obs, info0, draw_overlay=bool(args.draw_overlay)))
        ...
        for t in range(int(args.max_steps)):
            ...
            if interp_method == "none":
                ...
                for j in range(1, int(interp_n) + 1):
                    ...
                    _append(_grid_frame(obs_j, info_i, draw_overlay=False))
        ...
        writer.close()
        sim_s = float(t + 1) * float(step_dt_s)
        video_s = float(frames_written) / float(fps_used)
        print(
            f"   done={done} steps={t+1} sim_seconds~{sim_s:.2f}s "
            f"frames={frames_written} fps={fps_used:.3f} video_seconds~{video_s:.2f}s saved={out_path}"
        )
