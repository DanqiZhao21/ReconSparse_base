...
# Uses tools/smalltool/build_metrics_cache.py to write assets/nus/data/<scene>/env_cache.json
...
#   bash tools/smalltool/build_env_caches.sh
...
for ((i=START; i<=END; i++)); do
  ...
  if [[ "$VERBOSE" == "1" ]]; then
    "$python_bin" tools/smalltool/build_metrics_cache.py --scene "$i" --root "$ROOT" --version "$VERSION" --roi "$ROI" --cl_res "$CL_RES" --verbose || true
  else
    "$python_bin" tools/smalltool/build_metrics_cache.py --scene "$i" --root "$ROOT" --version "$VERSION" --roi "$ROI" --cl_res "$CL_RES" || true
  fi
