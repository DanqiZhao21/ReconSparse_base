...
RUN_ID="${RUN_ID:-$(date -u +%Y%m%d_%H%M%S)_reinforce_grpo_lane_dir_on}"
RUN_ROOT="$REPO_ROOT/outputs/reinforce_grpo_lane_dir_on/$RUN_ID"
PROMOTE_DIR="$REPO_ROOT/egoADs/SparseDriveV2/ckpt"
DATE_TAG="$(date -u +%Y%m%d)"

REINFORCE_CONFIG="$REPO_ROOT/script/configs/sparsedrive_v2/reinforcepp_closed_loop_sparsedrive_v2_grpo_lane_dir_on.yaml"
...
run_training \
  "reinforce_grpo_lane_dir_on" \
  "$REINFORCE_CONFIG" \
  "outputs/actor_learner_reinforcepp_grpo_lane_dir_on" \
  "reinforce_grpo_laneDirOn"

REINFORCE_CKPT="$(cat "$RUN_ROOT/reinforce_grpo_lane_dir_on_promoted_ckpt.txt")"
