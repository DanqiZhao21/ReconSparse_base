...
def default_run_specs() -> list[RunSpec]:
    ...
    return [
        RunSpec(
            run_key="reinforcepp_dac9_grpo_coef1",
            algo_tag="reinforcepp_dac9",
            coef=1.0,
            config_path=cfg_root / "reinforcepp_closed_loop_sparsedrive_v2_dac9_grpo_coef1.yaml",
        ),
        RunSpec(
            run_key="reinforcepp_dac9_grpo_coef01",
            algo_tag="reinforcepp_dac9",
            coef=0.1,
            config_path=cfg_root / "reinforcepp_closed_loop_sparsedrive_v2_dac9_grpo_coef01.yaml",
        ),
        RunSpec(
            run_key="reinforcepp_dac9_grpo_coef05",
            algo_tag="reinforcepp_dac9",
            coef=0.5,
            config_path=cfg_root / "reinforcepp_closed_loop_sparsedrive_v2_dac9_grpo_coef05.yaml",
        ),
    ]
...
def promote_checkpoint(*, src_ckpt: Path, spec: RunSpec, date_tag: str) -> tuple[Path, int]:
    ...
    version = detect_next_version(ckpt_dir=SPARSEDRIVE_CKPT_DIR, run_key_prefix="reinforcepp_dac9_grpo")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Reinforce++ DAC9-only GRPO coefficient sweep and HUGSIM-ORI eval.")
    parser.add_argument("--run-root", type=Path, default=REPO_ROOT / "outputs" / "reinforcepp_dac9_grpo_coef_sweep")
...
def main() -> int:
    args = parse_args()
    run_id = args.run_id or f"{time.strftime('%Y%m%d_%H%M%S', time.gmtime())}_reinforcepp_dac9_grpo_coef_sweep"
